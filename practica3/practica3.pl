...
contar_elementos([X|T], L, S) :- contar_elemento(X, L, C), concatena([X-C], D, S), contar_elementos(T, L, D), T \= [].
...
encode(L1, L2) :- dictionary(D), contar_elementos(D, L1, Laux), sort_list(Laux, LS), invierte(LS, LInv), build_tree(LInv, Tree), encode_list(L1, L2, Tree).
